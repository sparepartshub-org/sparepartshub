      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map(({ product, quantity }) => (
            <div key={product._id} className="card p-4 flex gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-steel-500 dark:text-gray-400">{product.brand} • {product.vehicleType === 'bike' ? '🏍️ Bike' : '🚗 Car'}</p>
        ...
        <div className="card p-6 h-fit sticky top-20">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-steel-500 dark:text-gray-400">Tax (18% GST)</span><span className="dark:text-gray-200">₹{tax.toLocaleString()}</span></div>
          ...
          {cartTotal < 2000 && (
            <p className="text-xs text-steel-400 dark:text-gray-500 mt-2">💡 Add ₹{(2000 - cartTotal).toLocaleString()} more for free shipping!</p>
          )}
